...
import { Download, HardDrive, Calendar, Gamepad2, Layers, ShieldCheck, MessageSquare, Send, User, Globe, Star, Pencil, Trash2, Sparkles, Image as ImageIcon, X, AlertTriangle, Crown, Ban, CornerDownRight, ChevronDown, CheckCircle2, Lock, Unlock, Timer, Loader2, ChevronRight, Home, Share2, Facebook, Twitter, Youtube, MonitorPlay, ExternalLink } from 'lucide-react';
import SEO from './SEO';
...
// Componente para manejar los comentarios (recursivo)
const CommentNode = ({ comment, replyingToId, setReplyingToId, setCommentError, isLoggedIn, isAdminComment, setIsAdminComment, replyName, setReplyName, replyText, setReplyText, handlePostReply }: any) => {
      ...
      <div className={`flex gap-4 p-4 rounded-2xl border ${comment.isAdmin ? 'bg-primary/5 border-primary/50' : 'bg-surface border-border-color/50'}`}>
        ...
        <div className="flex-1">
          <div className="flex flex-wrap items-center justify-between mb-1">
            ...
            <button onClick={() => setReplyingToId(isReplying ? null : comment.id)} className="text-xs font-bold text-text-muted hover:text-primary-hover flex items-center gap-1 transition-colors">
              <CornerDownRight size={14} /> Responder
            </button>
          </div>
          <p className={`leading-relaxed text-sm ${comment.isAdmin ? 'text-text-main font-medium' : 'text-text-muted'}`}>{comment.content}</p>
          {isReplying && (
...
const GameDetail: React.FC<GameDetailProps> = ({ game, allGames, onBack, onSelectGame, onSelectConsole, onHome, onEdit, onDelete, onReport, isLoggedIn }) => {
  ...
  useEffect(() => {
    ...
    if (isDownloadModalOpen && downloadTimer > 0) {
        ...
        // Start resolving if it's an AN1 link
  ...
  const handleFinalDownload = () => {
      // 1. Obtener el link base (dinámico o estático)
      let finalUrl = resolvedLink || game.downloadUrl;
      
      if (!finalUrl) return;

      // 2. Aplicar Acortador automático si existe en localStorage o es global
      const prefix = localStorage.getItem('shortener_prefix') || '';
      if (prefix && !finalUrl.includes(prefix)) {
                  ...
                  <div className="flex flex-col items-center text-center space-y-6">
                      {!isDownloadReady ? (
                          <>
                              ...
                              {isResolving && <p className="text-xs text-primary-hover font-bold animate-pulse">Extrayendo link de AN1...</p>}
                          </>
                      ) : (
                          <>
